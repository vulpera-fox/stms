...
public interface SseRepository {
    ...
    Map<String, SseEmitter> findAllStartWithById(String rcv_id); //해당 회원과  관련된 모든 Emitter를 찾는다
    
    Map<String, SseEmitter> findAllEmitterStartWithByEmailInList(List emails); //List 에서 해당 회원과  관련된 모든 Emitter를 찾는다(미 개발)
    
    Map<String, Object> findAllEventCacheStartWithById(String rcv_id); //해당 회원과관련된 모든 이벤트를 찾는다
    
    void deleteById(String rcv_id); //Emitter를 지운다
